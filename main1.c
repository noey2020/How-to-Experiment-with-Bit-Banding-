...
#define PortB_Bit6     (*((volatile unsigned long *) 0x42020800))
...
int main(void){
    while(1){
        /* 0b1000000 = 0x40 */
        GPIOB->ODR |= 0x00000040;                   /* Set and Clear the bit using port wide AND and OR instructions - Slowest method */
        delay();
        GPIOB->ODR &= !0x00000040;	  
        delay();
        bitAddress = & BitBand(&GPIOB->ODR, 6);			/* Use a run time macro to calculate the bit band alias address - 2nd Fastest method */
        *bitAddress = 1;														/* Set and Clear the bit */ 
        delay();
        *bitAddress = 0;
        delay();
        PortB_Bit6 = 1;															/* Set and Clear the bit using a pointer calculated at compile time - Fastest method */
        delay();
        PortB_Bit6  = 0;  
        delay();
    }
